...
const addToCart = async (req, res) => {
  try {
    const { productId, quantity, color, message } = req.body;
    ...
    if (cartItem) {
      ...
      const updateObject = { $inc: { "items.$.quantity": quantity } };

      // If message is available (not null or undefined), include it in the update object
      if (message !== null && message !== undefined) {
        updateObject.$set = { "items.$.message": message };
      }
      ...
      const iten = await UserCart.findOneAndUpdate(
        { userId: req.user.userId },
        { $push: { items: newItem } },
        { upsert: true }
      ).lean();
